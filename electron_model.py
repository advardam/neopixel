...
- Loads LED ring mapping from: /mnt/data/led_mapping.json
- Lets user select atom presets or enter atomic number
- Distributes electrons into classical shells (K,L,M,N) with capacities [2,8,18,32]
- Maps electrons onto physical rings (as listed in the mapping JSON)
- Allows toggling LEDs and exporting LED state to JSON

Run:
    python3 electron_model.py

Outputs:
    - Optionally writes LED state to /mnt/data/led_state_output.json when you click "Export"
"""
...
MAPPING_PATH = "/mnt/data/led_mapping.json"
...
class ElectronModelApp:
    ...
    def build_ui(self):
        ...
        self.preset_var = tk.IntVar(value=PRESETS[3][1])
        ...
        def on_preset_change(event):
            sel = preset_menu.get()
            # extract Z from string ending with (Z=number)
        ...
        ttk.Button(bottom, text="Export LED state to JSON", command=self.export_state).grid(row=0, column=0)
        ttk.Button(bottom, text="Clear LEDs", command=self.clear_leds).grid(row=0, column=1, padx=(8,0))
        ttk.Button(bottom, text="Exit", command=self.root.quit).grid(row=0, column=2, padx=(8,0))
    ...
    def export_state(self):
        ...
        default_path = "/mnt/data/led_state_output.json"
        try:
...
if __name__ == '__main__':
    try:
        ...
        print(f"Make sure the mapping JSON exists at: {MAPPING_PATH}")
        # fallback default
        mapping = {{ "ring_leds": [7,8,12,16] }}

    root = tk.Tk()
    app = ElectronModelApp(root, mapping)
    root.mainloop()
